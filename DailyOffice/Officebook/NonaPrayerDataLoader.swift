...
// MARK: - JSON 解析專用結構（純字串，因為 LocalizedJSONResolver 已經壓平）
struct NonaResponsoryJSON: Codable {
    let leader: String
    let people: String
}

struct NonaPrayerSectionJSON: Codable {
    let title: String?
    let rubric: String?
    let paragraphs: [String]
    let responses: [NonaResponsoryJSON]?
}

struct NonaPsalmAntiphonJSON: Codable {
    let season: String
    let text: String
    let fullText: String
}

struct NonaShortResponsorySetJSON: Codable {
    let title: String
    let responses: [NonaResponsoryJSON]
}

struct NonaReadingItemJSON: Codable {
    let season: String
    let content: String
...
// MARK: - 九時禱 JSON 根節點
struct NonaPrayerJSON: Codable {
    let openingNote: String
    let openingResponses: [NonaResponsoryJSON]
    let hymn: NonaPrayerSectionJSON
    let seasonalHymnEndings: [String: String]
    let bvmFeastAntiphon: String
    let psalmAntiphons: [NonaPsalmAntiphonJSON]
    let readings: [NonaReadingItemJSON]
    let shortResponsesOrdinary: NonaShortResponsorySetJSON
    let shortResponsesFeastEaster: NonaShortResponsorySetJSON
    let shortResponsesFeastOrdinary: NonaShortResponsorySetJSON
    let prayersSection: NonaPrayerSectionJSON
    let prayersResponses: [NonaResponsoryJSON]
    let collectOpening: [NonaResponsoryJSON]
    let nonaMemorialCollect: NonaPrayerSectionJSON
    let collectEndingResponses: [NonaResponsoryJSON]
    let closingText: NonaPrayerSectionJSON
}

// MARK: - 九時禱資料加載器
class NonaPrayerDataLoader {
    static let shared = NonaPrayerDataLoader()
    
    private var cache: [AppLanguage: NonaPrayerJSON] = [:]
    ...
    func load(language: AppLanguage? = nil) -> NonaPrayerJSON {
        let lang = language ?? currentLanguage
        if let cached = cache[lang] { return cached }
        
        guard let url = Bundle.main.url(forResource: "nona_prayer", withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let localizedData = LocalizedJSONResolver.resolve(data: data, language: lang),
              let decoded = try? JSONDecoder().decode(NonaPrayerJSON.self, from: localizedData) else {
            fatalError("❌ 無法載入 nona_prayer.json")
        }
        cache[lang] = decoded
        return decoded
    }
...
struct NonaPrayerData {
    private static var json: NonaPrayerJSON { NonaPrayerDataLoader.shared.load() }
    
    // 輔助轉換
    private static func map(_ r: NonaResponsoryJSON) -> Responsory {
        Responsory(leader: r.leader, people: r.people)
    }
    
    private static func map(_ s: NonaPrayerSectionJSON) -> PrayerSection {
        PrayerSection(title: s.title, rubric: s.rubric, paragraphs: s.paragraphs, responses: s.responses?.map { map($0) } ?? [])
    }
    
    // 對外暴露的 UI 資料
    static var openingNote: String { json.openingNote }
    static var openingResponses: [Responsory] { json.openingResponses.map { map($0) } }
    static var hymn: PrayerSection { map(json.hymn) }
    static var readings: [NonaReadingItemJSON] { json.readings }
    ...
    static var nonaMemorialCollect: PrayerSection { map(json.nonaMemorialCollect) }
    ...
    struct BVMFeastAntiphon {
        static var text: String { NonaPrayerData.json.bvmFeastAntiphon }
    ...
    static func psalmKeys(for date: Date) -> [String] {
        ...
        switch weekday {
        case 1: return ["詩篇 第126篇", "詩篇 第127篇", "詩篇 第128篇"]
        case 2, 5: return ["詩篇 第126篇"]
        case 3, 6: return ["詩篇 第127篇"]
        case 4, 7: return ["詩篇 第128篇"]
        default: return ["詩篇 第126篇"]
        }
    }
    ...
    struct SeasonalHymnEnding {
        ...
        static func assemble(baseVerses: [String], for date: Date, liturgy: DailyLiturgy) -> [String] {
            guard let key = endingKey(for: date, liturgy: liturgy),
                  let ending = NonaPrayerData.json.seasonalHymnEndings[key],
