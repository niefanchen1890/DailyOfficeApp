...
class TercePrayerViewModel: ObservableObject {
    ...
    var currentAntiphon: TercePrayerData.PsalmAntiphonUI? {
        let title = liturgy.mainTitle
        if TercePrayerData.BVMFeastAntiphon.isBVMFeast(title: title) {
            let text = TercePrayerData.BVMFeastAntiphon.text
            return TercePrayerData.PsalmAntiphonUI(season: isSimplified ? "圣母庆节" : "聖母慶節", text: text, fullText: text)
        }
        ...
        return TercePrayerData.psalmAntiphons.first { $0.season == localizedKey }
            ?? TercePrayerData.psalmAntiphons.first { $0.season == (isSimplified ? "全年通用" : "全年通用") }
    }

    var currentReading: TerceReadingItemJSON? {
        let readings = TercePrayerData.readings
        let matchingKey = selectedReadingOption.localizedTitle(isSimplified: isSimplified)
        return readings.first { $0.season == matchingKey }
    }

    var currentShortResponsorySet: TercePrayerData.ShortResponsorySetUI? {
        switch selectedShortResponseOption {
        case .ordinary: return TercePrayerData.shortResponsesOrdinary
        case .feast:    return isEasterSeason ? TercePrayerData.shortResponsesFeastEaster : TercePrayerData.shortResponsesFeastOrdinary
        }
    }
    ...
    var hymnLatinTitle: String? {
        let info = LiturgyCoreService.shared.getSeasonInfo(for: selectedDate)
        if (49...55).contains(info.daysFromEaster) { return "Veni, Creator Spiritus" }
        return nil
    }
...
struct TercePrayerView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(viewModel.isSimplified ? "三时祷" : "三時禱")
    ...
    private var hymnSection: some View {
        LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                if let title = TercePrayerData.hymn.title { SectionTitle(text: title) }
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(psalmKeys.indices, id: \.self) { index in
                    ...
                    if let psalm = PsalmsLoader.shared.psalmContent(for: key) {
                        let displayTitle = TercePrayerData.psalmDisplayTitle(for: key, isSimplified: isSimp)
    ...
    private var prayersSection: some View {
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                if let title = TercePrayerData.prayersSection.title { SectionTitle(text: title) }
                ...
                if viewModel.selectedPrayerOption == .show {
                    if let rubric = TercePrayerData.prayersSection.rubric { RubricBlock(text: rubric) }

                    Text(TercePrayerData.prayersSection.paragraphs[0])
                    ...
                    ForEach(TercePrayerData.prayersResponses, id: \.self) { r in
                        ResponsoryRow(response: r)
                    }
                } else {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            LiturgyCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Divider().padding(.vertical, 4)
                    RubricBlock(text: isSimp ? "¶ 诵念本日祝文，然后诵念以下纪念祝文。" : "¶ 誦唸本日祝文，然後誦唸以下紀念祝文。")
                    Text(TercePrayerData.dailyCollectText)
    ...
    private var endingSection: some View {
        VStack(spacing: 0) {
            Text(viewModel.isSimplified ? "❦ 三时祷至此结束。" : "❦ 三時禱至此結束。")
