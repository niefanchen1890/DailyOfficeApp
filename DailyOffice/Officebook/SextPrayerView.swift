            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(psalmKeys.indices, id: \.self) { index in
                    ...
                    if let psalm = PsalmsLoader.shared.psalmContent(for: key) {
                        let displayTitle = SextPrayerData.psalmDisplayTitle(for: key)
        ...
        LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if viewModel.selectedPrayerOption == .show {
                    if let rubric = SextPrayerData.prayersSection.rubric {
                    ...
                    let responses = SextPrayerData.prayersResponses
        ...
        VStack(alignment: .leading, spacing: 12) {
            LiturgyCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let rubric = SextPrayerData.sextMemorialCollect.rubric {
                        RubricBlock(text: rubric)
                    }

                    BodyText(SextPrayerData.sextMemorialCollect.paragraphs[0])
                }
            }
...
class SextPrayerViewModel: ObservableObject {
    ...
    /// 當季節對經（聖母慶節優先）
    var currentAntiphon: SextPrayerData.PsalmAntiphon? {
        let title = liturgy.mainTitle
        
        // 1️⃣ 聖母慶節優先（完整名稱列表匹配）
        if SextPrayerData.BVMFeastAntiphon.isBVMFeast(title: title) {
            return SextPrayerData.PsalmAntiphon(
                season: "聖母慶節",
                text: SextPrayerData.BVMFeastAntiphon.text
            )
        }
        ...
        return SextPrayerData.psalmAntiphons.first { $0.season == key }
            ?? SextPrayerData.psalmAntiphons.first { $0.season == "全年通用" }
    }

    func fullAntiphonText(for antiphon: SextPrayerData.PsalmAntiphon) -> String {
        // 聖母慶節專用
        ...
        case "全年通用":
            return "住在天上的主阿，求你憐憫我們。"
        case "降臨期":
            return "看哪，萬國所羨慕的必來到，主的殿必充滿榮耀。哈利路亞。"
        case "聖誕期":
            return "看哪，馬利亞為我們誕生了救主，當約翰看見他，就說：看哪，上帝的羔羊，除去世人的罪的！哈利路亞。"
        case "大齋期":
            return "當以恆久的忍耐，不懈的齋戒，公義的兵器，顯明我們是屬上帝的。"
        case "復活期":
    ...
    /// 當日讀經
    var currentReading: SextPrayerData.SextReadingItem? {
        SextPrayerData.SextReadingsLoader.shared.item(for: selectedReadingOption.rawValue)
    }

    /// 當日簡短啟應組
    var currentShortResponsorySet: SextPrayerData.ShortResponsorySet? {
        switch selectedShortResponseOption {
        case .ordinary:
            return SextPrayerData.shortResponsesOrdinary
        case .feast:
            // 復活期內顯示哈利路亞，復活期外隱藏
            return isEasterSeason
                ? SextPrayerData.shortResponsesFeastEaster
                : SextPrayerData.shortResponsesFeastOrdinary
        }
    }
    ...
    var hymnVerses: [String] {
        SextPrayerData.SeasonalHymnEnding.assemble(
...
// MARK: - 預覽
struct SextPrayerView_Previews: PreviewProvider {
