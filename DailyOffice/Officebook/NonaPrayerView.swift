...
struct NonaPrayerView: View {
    ...
    private var openingNoteSection: some View {
        LiturgyCard {
            RubricBlock(text: NonaPrayerData.openingNote)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(psalmKeys.indices, id: \.self) { index in
                    ...
                    if let psalm = PsalmsLoader.shared.psalmContent(for: key) {
                        let displayTitle = NonaPrayerData.psalmDisplayTitle(for: key)
        ...
        LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if viewModel.selectedPrayerOption == .show {
                    if let rubric = NonaPrayerData.prayersSection.rubric {
                    ...
                    let responses = NonaPrayerData.prayersResponses
        ...
        VStack(alignment: .leading, spacing: 12) {
            LiturgyCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let rubric = NonaPrayerData.nonaMemorialCollect.rubric {
                        RubricBlock(text: rubric)
                    }

                    BodyText(NonaPrayerData.nonaMemorialCollect.paragraphs[0])
                }
            }
...
class NonaPrayerViewModel: ObservableObject {
    ...
    /// 當季節對經（聖母慶節優先）
    var currentAntiphon: NonaPrayerData.PsalmAntiphon? {
        let title = liturgy.mainTitle
        
        // 1️⃣ 聖母慶節優先（完整名稱列表匹配）
        if NonaPrayerData.BVMFeastAntiphon.isBVMFeast(title: title) {
            return NonaPrayerData.PsalmAntiphon(
                season: "聖母慶節",
                text: NonaPrayerData.BVMFeastAntiphon.text
            )
        }
        ...
        return NonaPrayerData.psalmAntiphons.first { $0.season == key }
            ?? NonaPrayerData.psalmAntiphons.first { $0.season == "全年通用" }
    }

    func fullAntiphonText(for antiphon: NonaPrayerData.PsalmAntiphon) -> String {
        // 聖母慶節專用
        ...
        case "全年通用":
            return "敬畏主的人，就有福了。"
        case "降臨期":
            return "你全能的道從天上君尊寶座降下來，哈利路亞。"
        case "聖誕期":
            return "摩西看見的焚而不毀的荊棘叢，是你保全童貞的標記，令人讚頌不止。上帝聖母，求你為我們代禱。"
        case "大齋期":
            return "當以上帝公義的能力為盔甲，在百般的忍耐上，顯明我們是屬上帝的。"
        case "復活期":
    ...
    /// 當日讀經
    var currentReading: NonaPrayerData.NonaReadingItem? {
        NonaPrayerData.NonaReadingsLoader.shared.item(for: selectedReadingOption.rawValue)
    }

    /// 當日簡短啟應組
    var currentShortResponsorySet: NonaPrayerData.ShortResponsorySet? {
        switch selectedShortResponseOption {
        case .ordinary:
            return NonaPrayerData.shortResponsesOrdinary
        case .feast:
            // 復活期內顯示哈利路亞，復活期外隱藏
            return isEasterSeason
                ? NonaPrayerData.shortResponsesFeastEaster
                : NonaPrayerData.shortResponsesFeastOrdinary
        }
    }
    ...
    var hymnVerses: [String] {
        NonaPrayerData.SeasonalHymnEnding.assemble(
...
// MARK: - 預覽
struct NonaPrayerView_Previews: PreviewProvider {
