...
// MARK: - 視圖模型
class NonaPrayerViewModel: ObservableObject {
    @Published var selectedDate: Date = Date()
    @Published var selectedReadingOption: NonaReadingOption = .ordinary
    @Published var selectedShortResponseOption: NonaShortResponseOption = .ordinary
    @Published var selectedPrayerOption: NonaPrayerOption = .show
    ...
    var currentAntiphon: NonaPrayerData.PsalmAntiphonUI? {
        let title = liturgy.mainTitle
        if NonaPrayerData.BVMFeastAntiphon.isBVMFeast(title: title) {
            let text = NonaPrayerData.BVMFeastAntiphon.text
            return NonaPrayerData.PsalmAntiphonUI(season: isSimplified ? "圣母庆节" : "聖母慶節", text: text, fullText: text)
        }
        ...
        return NonaPrayerData.psalmAntiphons.first { $0.season == localizedKey }
            ?? NonaPrayerData.psalmAntiphons.first { $0.season == (isSimplified ? "全年通用" : "全年通用") }
    }

    var currentReading: NonaReadingItemJSON? {
        let readings = NonaPrayerData.readings
        let matchingKey = selectedReadingOption.localizedTitle(isSimplified: isSimplified)
        return readings.first { $0.season == matchingKey }
    }

    var currentShortResponsorySet: NonaPrayerData.ShortResponsorySetUI? {
        switch selectedShortResponseOption {
        case .ordinary: return NonaPrayerData.shortResponsesOrdinary
        case .feast:    return isEasterSeason ? NonaPrayerData.shortResponsesFeastEaster : NonaPrayerData.shortResponsesFeastOrdinary
        }
    }
    ...
    var hymnVerses: [String] {
        return NonaPrayerData.SeasonalHymnEnding.assemble(
            baseVerses: NonaPrayerData.hymn.paragraphs,
            for: selectedDate,
            liturgy: liturgy
        )
    }
...
struct NonaPrayerView: View {
    @StateObject private var viewModel: NonaPrayerViewModel
    @Environment(\.colorScheme) var colorScheme
    let date: Date
    
    init(date: Date = Date()) {
        self.date = date
        let vm = NonaPrayerViewModel()
    ...
    var body: some View {
        ...
        .navigationTitle(viewModel.isSimplified ? "九时祷" : "九時禱")
    ...
    private var openingNoteSection: some View {
        LiturgyCard { RubricBlock(text: NonaPrayerData.openingNote) }
    }
    ...
    private var openingResponsesSection: some View {
        LiturgyCard {
            VStack(alignment: .leading, spacing: 10) {
                ForEach(NonaPrayerData.openingResponses, id: \.self) { r in
    ...
    private var hymnSection: some View {
        LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                if let title = NonaPrayerData.hymn.title { SectionTitle(text: title) }
    ...
    private var psalmsSection: some View {
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                let psalmKeys = NonaPrayerData.psalmKeys(for: viewModel.selectedDate)
                ForEach(psalmKeys.indices, id: \.self) { index in
                    let key = psalmKeys[index]
                    if let psalm = PsalmsLoader.shared.psalmContent(for: key) {
                        let displayTitle = NonaPrayerData.psalmDisplayTitle(for: key, isSimplified: isSimp)
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Picker(isSimp ? "读经选择" : "讀經選擇", selection: $viewModel.selectedReadingOption) {
                    ForEach(NonaReadingOption.allCases, id: \.self) { option in
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Picker(isSimp ? "启应选择" : "啟應選擇", selection: $viewModel.selectedShortResponseOption) {
                    ForEach(NonaShortResponseOption.allCases, id: \.self) { option in
                        Text(option.localizedTitle(isSimplified: isSimp)).tag(option)
                    }
                }
                .pickerStyle(.segmented).padding(.vertical, 6)
    ...
    private var prayersSection: some View {
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                if let title = NonaPrayerData.prayersSection.title { SectionTitle(text: title) }

                Picker(isSimp ? "祈祷选择" : "祈禱選擇", selection: $viewModel.selectedPrayerOption) {
                    ForEach(NonaPrayerOption.allCases, id: \.self) { option in
                        Text(option.localizedTitle(isSimplified: isSimp)).tag(option)
                    }
                }
                .pickerStyle(.segmented).padding(.vertical, 6)
                ...
                if viewModel.selectedPrayerOption == .show {
                    if let rubric = NonaPrayerData.prayersSection.rubric { RubricBlock(text: rubric) }

                    Text(NonaPrayerData.prayersSection.paragraphs[0])
                    ...
                    BodyText(NonaPrayerData.prayersSection.paragraphs[1])

                    Divider().padding(.vertical, 6)
                    ForEach(NonaPrayerData.prayersResponses, id: \.self) { r in
                        ResponsoryRow(response: r)
                    }
                } else {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            LiturgyCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    ForEach(NonaPrayerData.collectOpening, id: \.self) { r in
                        ResponsoryRow(response: r)
                    }
                    ...
                    if let rubric = NonaPrayerData.nonaMemorialCollect.rubric { RubricBlock(text: rubric) }
                    if !NonaPrayerData.nonaMemorialCollect.paragraphs.isEmpty {
                        Text(NonaPrayerData.nonaMemorialCollect.paragraphs[0])
            ...
            LiturgyCard {
                VStack(alignment: .leading, spacing: 10) {
                    ForEach(NonaPrayerData.collectEndingResponses, id: \.self) { r in
                        ResponsoryRow(response: r)
                    }
                }
            }
    ...
    private var closingSection: some View {
        ...
        return LiturgyCard {
            VStack(alignment: .leading, spacing: 14) {
                if NonaPrayerData.closingText.paragraphs.count > 0 {
                    let text = NonaPrayerData.closingText.paragraphs[0]
    ...
    private var endingSection: some View {
        VStack(spacing: 0) {
            Text(viewModel.isSimplified ? "❦ 九时祷至此结束。" : "❦ 九時禱至此結束。")
