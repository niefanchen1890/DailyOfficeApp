...
struct SextResponsoryJSON: Codable {
    let leader: String
    let people: String
}

struct SextPrayerSectionJSON: Codable {
    let title: String?
    let rubric: String?
    let paragraphs: [String]
    let responses: [SextResponsoryJSON]?
}

struct SextPsalmAntiphonJSON: Codable {
    let season: String
    let text: String
    let fullText: String
}

struct SextShortResponsorySetJSON: Codable {
    let title: String
    let responses: [SextResponsoryJSON]
}

struct SextReadingItemJSON: Codable {
    let season: String
    let content: String
...
// MARK: - 六時禱 JSON 根節點
struct SextPrayerJSON: Codable {
    let openingResponses: [SextResponsoryJSON]
    let hymn: SextPrayerSectionJSON
    let seasonalHymnEndings: [String: String]
    let bvmFeastAntiphon: String
    let psalmAntiphons: [SextPsalmAntiphonJSON]
    let readings: [SextReadingItemJSON]
    let shortResponsesOrdinary: SextShortResponsorySetJSON
    let shortResponsesFeastEaster: SextShortResponsorySetJSON
    let shortResponsesFeastOrdinary: SextShortResponsorySetJSON
    let prayersSection: SextPrayerSectionJSON
    let prayersResponses: [SextResponsoryJSON]
    let collectOpening: [SextResponsoryJSON]
    let sextMemorialCollect: SextPrayerSectionJSON
    let collectEndingResponses: [SextResponsoryJSON]
    let closingText: SextPrayerSectionJSON
}

// MARK: - 六時禱資料加載器
class SextPrayerDataLoader {
    static let shared = SextPrayerDataLoader()
    
    private var cache: [AppLanguage: SextPrayerJSON] = [:]
    ...
    func load(language: AppLanguage? = nil) -> SextPrayerJSON {
        let lang = language ?? currentLanguage
        if let cached = cache[lang] { return cached }
        
        guard let url = Bundle.main.url(forResource: "sext_prayer", withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let localizedData = LocalizedJSONResolver.resolve(data: data, language: lang),
              let decoded = try? JSONDecoder().decode(SextPrayerJSON.self, from: localizedData) else {
            fatalError("❌ 無法載入 sext_prayer.json")
        }
        cache[lang] = decoded
        return decoded
    }
...
struct SextPrayerData {
    private static var json: SextPrayerJSON { SextPrayerDataLoader.shared.load() }
    
    // 輔助轉換
    private static func map(_ r: SextResponsoryJSON) -> Responsory {
        Responsory(leader: r.leader, people: r.people)
    }
    
    private static func map(_ s: SextPrayerSectionJSON) -> PrayerSection {
    ...
    static var readings: [SextReadingItemJSON] { json.readings }
    ...
    static var sextMemorialCollect: PrayerSection { map(json.sextMemorialCollect) }
    ...
    struct BVMFeastAntiphon {
        static var text: String { SextPrayerData.json.bvmFeastAntiphon }
    ...
    static func psalmKeys(for date: Date) -> [String] {
        ...
        switch weekday {
        case 1: return ["詩篇 第123篇", "詩篇 第124篇", "詩篇 第125篇"]
        case 2, 5: return ["詩篇 第123篇"]
        case 3, 6: return ["詩篇 第124篇"]
        case 4, 7: return ["詩篇 第125篇"]
        default: return ["詩篇 第123篇"]
        }
    }
    ...
    struct SeasonalHymnEnding {
        ...
        static func assemble(baseVerses: [String], for date: Date, liturgy: DailyLiturgy) -> [String] {
            guard let key = endingKey(for: date, liturgy: liturgy),
                  let ending = SextPrayerData.json.seasonalHymnEndings[key],
